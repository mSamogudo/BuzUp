...
class TicketPageView(APIView):
    """A pagina que o passageiro abre a partir do SMS.

    Este endereco devolvia o PDF: 339 KB, dos quais 268 sao a imagem de fundo.
    Gerar custava 100 ms — o servidor nunca foi o problema. Era o tamanho: num
    telemovel com dados moveis sao segundos a olhar para um ecra branco, na
    paragem, com o autocarro a chegar.

    Passa a devolver uma pagina de ~8 KB, sem um unico ficheiro a ir buscar. O
    PDF continua em `/pdf/`, ligado a partir daqui, para quem o quiser imprimir.

    Os links ja enviados apontam para aqui e passam a abrir depressa sem que
    ninguem tenha de fazer nada.
    """
    ...
    def get(self, request, token):
        ...
        from apps.guest_checkouts.ticket_page import render_ticket_page

        tp = _bilhete_por_token(token)
        if tp is None:
            return DjangoHttpResponse(
                "<!doctype html><meta charset=utf-8>"
                "<p style='font:16px system-ui;padding:24px'>Bilhete nao encontrado.</p>",
                content_type="text/html; charset=utf-8", status=404)

        operadora, emergencia = _identidade_da_operadora()
        html_str = render_ticket_page(
            tp,
            pdf_url=request.build_absolute_uri("pdf/"),
            operadora=operadora,
            emergencia=emergencia,
        )
        resposta = DjangoHttpResponse(html_str, content_type="text/html; charset=utf-8")
        # O bilhete nao muda depois de emitido. Deixar o telemovel guarda-lo
        # significa que reabrir na fila do embarque e instantaneo, e funciona
        # sem rede — que e quando mais faz falta.
        resposta["Cache-Control"] = "private, max-age=3600"
        return resposta


def _bilhete_por_token(token: str):
    import hashlib
    ...
    if tp is not None:
        # A pagina desenha o QR a partir do token em claro, que nao esta
        # guardado: so chega no URL.
        tp.token = token
    return tp


def _identidade_da_operadora() -> tuple:
    """Nome e linha de emergencia, para o rodape da pagina."""
    try:
        from apps.branding.models import BrandingSettings

        b = BrandingSettings.objects.first()
        if b:
            return (getattr(b, "company_name", "") or "",
                    getattr(b, "emergency_phone", "") or "")
    except Exception:
        pass
    return ("", "")
