...
export default function VehiclesPage({ embedded }: { embedded?: boolean }) {
  ...
  const [form, setForm] = useState({ registration: "", make: "", model_name: "", seated_capacity: "0", standing_capacity: "0", seat_layout: "2+2", last_row_seats: "0", status: "active" });

  const reset = () => { setEditId(null); setModalOpen(false); setLivrete(null); setForm({ registration: "", make: "", model_name: "", seated_capacity: "0", standing_capacity: "0", seat_layout: "2+2", last_row_seats: "0", status: "active" }); };
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (livrete) {
        ...
        fd.append("seat_layout", form.seat_layout);
        fd.append("last_row_seats", String(Number(form.last_row_seats)));
